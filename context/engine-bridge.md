...
5. **800-171 Rev 2 as the rendered profile of record** for the CMMC work, per the [runbook](../runbooks/cmmc-l2-self-assessment.md), with the SPRS score derived from control state rather than from a spreadsheet nobody can reproduce. This is now a shipped engine capability, not a promise: the [110-requirement workbook](../assessment/nist-800-171-rev2-workbook.md) is rendered, and the [FedRAMP 20x KSI map](../assessment/fedramp-20x-ksi-map.md) scores the control set against all 46 CR26 indicators for the day the own-listing question resolves.
