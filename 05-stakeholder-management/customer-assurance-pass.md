...
The end-state for the answer bank: each answer carries a pointer to the control it rests on and, where one exists, the evidence artifact behind that control. This is the stakeholder-management pattern from [the engine](../generated/engine-bridge.md): one posture, rendered per audience, from a single source of truth. Once wired, three things fall out:
