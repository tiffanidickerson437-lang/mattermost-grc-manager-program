...
The human-readable framework renderings. Each document here is a view of the same underlying control set, rendered into the language one framework's audience needs. The machine-readable mappings and the tools that check these against live upstream sources sit in [`04-evidence-and-audit/data/`](../data/).
...
| [`fedramp-20x-ksi-map.md`](fedramp-20x-ksi-map.md) | The 46 FedRAMP 20x Key Security Indicators mapped to the control set, reconciled against FedRAMP's live ruleset by the [drift check](../upstream-conformance-receipt.md) |
