...
- **What that means for rigor:** the score we post to SPRS is a signed claim to the federal government with False Claims Act exposure and an active DOJ enforcement initiative behind it. Losing the outside auditor actually raises the internal evidence bar, since nobody now stands between the company's claim and the government reading it.

## Step 1: Boundary (this decides everything)

Document the assessment boundary before assessing anything:

1. Which entity holds the DFARS-clause contracts (parent, the federal subsidiary, or both), building on the entity answer from day one.
...
Each requirement gets exactly one of three dispositions:
...
| **Not met** | Goes on the POA&M with an owner, a date, and an interim risk note. An unmet requirement with a credible plan is defensible, while an overstated "met" is where the False Claims Act exposure lives, so honesty here is the safer position as well as the right one |
| **Not applicable** | Documented rationale tied to the boundary, because an N/A without a written rationale tends to become a finding at the worst possible time |
...
3. Nothing gets affirmed that cannot be re-derived on demand, which is really the principle the whole runbook exists to protect.
